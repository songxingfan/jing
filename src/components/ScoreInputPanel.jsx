...
export default function ScoreInputPanel({ currentExam, onUpdateExam }) {
  ...
  const [saveStatus, setSaveStatus] = useState({ saving: false, lastSave: null, error: null });
  ...
  const handleSave = useCallback(async () => {
    ...
    if (!assetsLoaded || !window.luckysheet) {
    ...
    try {
      ...
      // 构建保存数据
      const saveData = allSheets.map((sheet, index) => {
        const sheetData = {
      ...
      if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
        wsRef.current.send(JSON.stringify(savePayload));
        ...
        setTimeout(() => {
          if (saveStatus.saving) {
            ...
            message.error("保存超时，请重试");
          }
        }, 10000);
      } else {
        // 如果WebSocket不可用，尝试通过HTTP API保存
        try {
          ...
          console.error('[ScoreInputPanel] API save failed:', apiError);
          setSaveStatus(prev => ({ 
            ...prev, 
            saving: false, 
            error: apiError.message || "保存失败" 
          }));
          message.error("保存失败: " + (apiError.message || "未知错误"));
        }
      }
    } catch (error) {
      console.error('[ScoreInputPanel] Save error:', error);
      setSaveStatus(prev => ({ 
        ...prev, 
        saving: false, 
  ...
  }, [fileInfo, assetsLoaded, getCurrentSheetIndex, saveStatus.saving]);
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "70vh" }}>
      <div style={{ marginBottom: 12 }}>
        ...
        <Button
          ...
          style={{ marginRight: 8 }}
        >
          <Icon type="save" /> {saveStatus.saving ? "保存中..." : "保存"}
        </Button>
        {!assetsLoaded && <span style={{ marginLeft: 12, color: "#faad14" }}>正在加载编辑器资源...</span>}
        {saveStatus.saving && <span style={{ marginLeft: 12, color: "#52c41a" }}>正在保存表格数据...</span>}
        {saveStatus.lastSave && (
          <span style={{ marginLeft: 12, color: "#1890ff", fontSize: "12px" }}>
            最后保存: {new Date(saveStatus.lastSave.timestamp).toLocaleTimeString()}
          </span>
        )}
        {saveStatus.error && (
          <span style={{ marginLeft: 12, color: "#ff4d4f", fontSize: "12px" }}>
